...
>
	...
	<header class="text-white sticky top-0 z-50">
		<div class="px-4 md:px-8 lg:px-20 py-5">
			<nav class="px-6 py-4 flex items-center justify-between">
				...
				<div class="w-auto gap-x-[40px] items-center justify-between hidden min-[680px]:flex">
					{#if data.user}
						<a
							href="/chat"
							class="font-semibold border-none cursor-pointer text-white hover:text-gray-300 transition"
						>
							Your Speakeasy
						</a>
						<form method="POST" action="?/logout">
							<button class="bg-gray-900 px-6 py-2 rounded-full hover:bg-neutral-800 transition"
								>Log out</button
							>
						</form>
					{:else}
						...
						<a
							href="/login"
							class="bg-gray-900 px-6 py-2 rounded-full hover:bg-neutral-800 transition"
						>
							Log in
						</a>
					{/if}
				...
				<div class="min-[680px]:hidden items-center" on:click={openHamDropdown}>
					<svg
	...
	<section class="w-full h-full flex flex-col justify-between items-center">
		<div class="flex flex-1 flex-col pt-8 px-8 md:px-12 lg:pl-28 w-full mr-auto">
			...
			<div class="flex justify-center min-[500px]:justify-start">
				<a
					href="/login"
					class="text-lg bg-white font-semibold px-6 py-4 rounded-full hover:bg-green-600 transition"
				>
					{#if data.user}
						Enter your Speakeasy, {data.user.first_name}
					{:else}
						Join the Speakeasy
					{/if}
				</a>
	...
	<footer class="mt-auto">
		<div class="flex flex-col justify-center items-center mb-5 gap-1">
			...
			<span class="text-white text-xs">Code inquiry or bugs: speakeaisy8@gmail.com</span>
		</div>
	</footer>
